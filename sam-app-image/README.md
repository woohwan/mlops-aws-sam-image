...
```bash
sam-app-image$ sam local invoke InferenceFunction --event events/event.json
```
...
sam-app-image$ curl http://localhost:3000/classify_digit
```
...
```bash
sam-app-image$ sam logs -n InferenceFunction --stack-name "sam-app-image" --tail
```
...
```bash
sam delete --stack-name "sam-app-image"
```
